...
try:
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS user_in (id_user VARCHAR(15), timestamp TIMESTAMP, x DECIMAL(4,2), y DECIMAL(4,2), aoi INT(2));")
    cursor.execute("SELECT * FROM user_in;")
    num_user_in = cursor.rowcount

    cursor.execute("CREATE TABLE IF NOT EXISTS user_out (id_user VARCHAR(15), timestamp TIMESTAMP, x DECIMAL(4,2), y DECIMAL(4,2), near_aoi INT(2));")
             ...
             while iter:
                 ...
                 if id_selector < num_id_distinct + 1 and id_selector != 0:
                    ...
                    for dot in dot_id:
                        ...
                        aoi_user_in = cursor.fetchone() - 1
                        num_aoi_user_in = cursor.rowcount
                        if num_aoi_user_in == 1:
                           print "...ATTENDERE... Righe da processare: ", num_dot
                           aoi_list.append(aoi_user_in)
                        else:
                           cursor.execute("SELECT near_aoi FROM user_out WHERE id_user = %s AND timestamp = %s AND x = %s AND y = %s;", (dot[0], dot[1], dot[2], dot[3]))
                           aoi_user_out = cursor.fetchone() - 1
                           print "...ATTENDERE... Righe da processare: ", num_dot
                           aoi_list.append(aoi_user_out)
                        num_dot = num_dot - 1
                    print aoi_list
                    iter = False
                 elif id_selector == 0:
                    print "hai selezionato tutti"
                 else:
